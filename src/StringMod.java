...
public class StringMod {
	public ArrayList<String> stringArrayCutter(ArrayList<String> arrayIn,
	...
	public boolean ifStartsWithVowel(String in) {
		...
		if (in.startsWith("a") || in.startsWith("A") || in.startsWith("i")
				|| in.startsWith("I") || in.startsWith("o")
				|| in.startsWith("O") || in.startsWith("u")
				|| in.startsWith("U") || in.startsWith("e")
				|| in.startsWith("E")) {
	...
	public String stringToEffectsTarget(String target) {
		...
		String converted = "", tokenStor, capitalStor;
		...
		if (target.contains(" ")) {
			...
			do {
				...
				converted += tokenStor.replaceFirst(
						String.valueOf(tokenStor.charAt(0)),
						capitalStor.toUpperCase());
	...
	public String pluralize(String input){
		if (input.endsWith("s")
				|| input.endsWith("h")
				&& input.endsWith("gh") == false
				|| input.endsWith("x")
				|| input.endsWith("o")) {
			input = (input + "es");
		} else {
			if (input.endsWith("y")) {
				if (ifStartsWithVowel(String.valueOf(input.charAt(
						input.length() - 2)))) {
					input = (input + "s");
				} else {
					input = (
