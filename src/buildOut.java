...
public class BuildOut {
	public String buildCommOut(ArrayList<String> commandIn, String output,
			String input, String doer) {
		...
		try {
			if (commandIn.get(1).isEmpty()) {
				if (doer.equals("player")) {
					...
					if (commandIn.get(0).endsWith("s")
							...
							|| commandIn.get(0).endsWith("o")) {
						output += (doer + " " + commandIn.get(0) + "es");
					} else {
						if (commandIn.get(0).endsWith("y")) {
							if (sM.ifStartsWithVowel(String.valueOf(commandIn
									.get(0).charAt(
										...
										commandIn.get(0).replace(
												commandIn.get(0).charAt(
														commandIn.get(0)
																.length() - 2)
														+ "y",
												String.valueOf(commandIn.get(0)
														.charAt(commandIn
																.get(0)
																.length() - 2))
														+ ""));
				...
				if (doer.equals("player")) {
					output += ("You " + commandIn.get(0) + " and then " + commandIn
							.get(1));
				} else {
					if (commandIn.get(0).endsWith("s")
							|| commandIn.get(0).endsWith("h")
					...
					if (commandIn.get(1).endsWith("s")
							|| commandIn.get(1).endsWith("h")
							&& commandIn.get(1).endsWith("gh") == false
							|| commandIn.get(1).endsWith("x")) {
						output += (commandIn.get(1) + "es");
					} else {
						if (commandIn.get(1).endsWith("y")) {
							if (sM.ifStartsWithVowel(String.valueOf(commandIn
									.get(1).charAt(
											commandIn.get(1).length() - 2)))) {
								output += (doer + " " + commandIn.get(1) + "s");
							} else {
								commandIn.set(
										1,
										commandIn.get(1).replace(
												commandIn.get(1).charAt(
														commandIn.get(1)
																.length() - 2)
														+ "y",
												String.valueOf(commandIn.get(1)
														.charAt(commandIn
																.get(1)
																.length() - 2))
														+ ""));
			...
			if (doer.equals("player")) {
				...
				if (commandIn.get(0).endsWith("s")
					...
					if (commandIn.get(0).endsWith("y")) {
						if (sM.ifStartsWithVowel(String.valueOf(commandIn
								.get(0).charAt(commandIn.get(0).length() - 2)))) {
							output += (doer + " " + commandIn.get(0) + "s");
						} else {
							commandIn.set(
									0,
	...
	public String buildCharOut(ArrayList<String> charsIn, String output,
			String input, String doer) {
		try {
			if (charsIn.get(1).isEmpty()) {
				if (charsIn.get(0).equals("player")
						|| charsIn.get(0).equals("me")
						|| charsIn.get(0).equals("myself")) {
					if (doer.equals("player")) {
						output += (" yourself");
					} else {
						output += (" you");
			...
			if (charsIn.get(0).equals("player") || charsIn.get(0).equals("me")
					|| charsIn.get(0).equals("myself")) {
				if (doer.equals("player")) {
					output += (" yourself");
				} else {
	...
	public String buildObjOut(ArrayList<String> objIn, String output,
			String input) {
		StringMod sM = new StringMod();
	...
	public String buildUseOut(ArrayList<String> useLinkingIn, String output,
			String input) {
		try {
	...
	public String buildToTarOut(ArrayList<String> toLinkingIn, String output,
			String input) {
		try {
