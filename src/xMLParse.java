...
public class XMLParse
{
	...
	ArrayList<String> tStrD = new ArrayList<String>(),
					  ...
					  tBooD = new ArrayList<String>(),	//f==false t==true n==no change (null)
					  tBooE = new ArrayList<String>(),	//f==false t==true n==no change (null)
					  tIntEEqu = new ArrayList<String>(),
					  tID1 = new ArrayList<String>(),
					  tID2 = new ArrayList<String>(),
					  tID3 = new ArrayList<String>(),
					  tAID = new ArrayList<String>(),
	 				  tLLID = new ArrayList<String>(),
	...
	public void xMLParser(String dir) throws IOException, InterruptedException
	{
		...
		{
			xMLNextTag(readIn, 0);
			while(readIn.contains(tTagA.get(0)) == false) //loops until readIn = closing tag
			{
				readIn = bis.readLine();
				if(readIn.contains("=") == false && readIn.contains("\"") == false && readIn.contains(tTagA.get(0)) == false) //dataType Level
				{
					xMLNextTag(readIn, 1);
					while(readIn.contains(tTagA.get(1)) == false)
					{
						readIn = bis.readLine();
						if(readIn.contains("=") == false && readIn.contains("\"") == false && readIn.contains(tTagA.get(1)) == false && readIn.contains(tTagA.get(0)) == false) //category Level
						{
							xMLNextTag(readIn, 2);
							while(readIn.contains(tTagA.get(2)) == false)
							{
								readIn = bis.readLine();
								if(readIn.contains("=") == false && readIn.contains("\"") == false && readIn.contains(tTagA.get(2)) == false  && readIn.contains(tTagA.get(1)) == false  && readIn.contains(tTagA.get(0)) == false) //subCategory Level
								{
									xMLNextTag(readIn, 3);
									while(readIn.contains(tTagA.get(3)) == false)
									{
										...
										if(readIn.contains("=") == false && readIn.contains("\"") == false && readIn.contains(tTagA.get(3)) == false && readIn.contains(tTagA.get(2)) == false  && readIn.contains(tTagA.get(1)) == false  && readIn.contains(tTagA.get(0)) == false) //iD1 Level
										{
											...
											{
												if(readIn.contains(tID1.get(0)))
												{
													xMLNextTag(readIn, 4);
													while(readIn.contains(tTagA.get(4)) == false)
													{
														readIn = bis.readLine();
														if(readIn.contains("=") == false && readIn.contains("\"") == false)
														{
															...
															{
																if(readIn.contains(tID2.get(0)))
																{
																	xMLNextTag(readIn, 5);
																	while(readIn.contains(tTagA.get(5)) == false)
																	{
																		readIn = bis.readLine();
																		...
																		{
																			if(readIn.contains("</") || readIn.contains("_"))
																			{
																			}
																			else
																			{
	...
	public int toDoParse(String dir, String readIn, int i, BufferedReader bis) throws IOException
	{
		...
		{
			readIn = bis.readLine();
			if(readIn.contains("=") == false && readIn.contains("\"") == false && readIn.contains("_") == false || readIn.contains("</toDo>")) //iDToDo1 Level
			{
			}
			else
			{
				...
				{	
					...
					if(readIn.contains("+") || readIn.contains("-"))
					{	
						if(readIn.contains("+") && readIn.contains("=") == false)
						{
	...
	public boolean arrayElementCheck(String toCheck, int element, ArrayList<String> array)
	{
