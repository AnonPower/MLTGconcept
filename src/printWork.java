...
public class PrintWork {
	...
	public void printWhoIsHere(String dir) throws IOException,
		...
		try {
			do {
				if (xP.getTStrD().get(x).equals("player")
						|| xP.getTStrD().get(x).equals("me")
						|| xP.getTStrD().get(x).equals("myself")
						|| xP.getTStrD().get(x).equals("self")) {
					y++;
				} else {
		...
		if (output.endsWith("are here.\n")) {
			...
			if (output.contains(" and ")) {
				...
				output = output.replace(" " + xP.getTStrD().get(x - y) + ",",
						" and " + xP.getTStrD().get(x - y) + " are here.\n");
				System.out.print(output);
			}
		}
	}

	public void printWhatIsHere(String dir) throws IOException,
			InterruptedException {
		XMLParse xP = new XMLParse();
			...
			do {
				if (x == xP.getTStrD().size() - 2) {
					if (sM.ifStartsWithVowel(xP.getTStrD().get(x))) {
						System.out
								.print("an " + xP.getTStrD().get(x) + " and ");
					} else {
	...
	// when called upon, activated by the playerCommand ="help" in the
	// setPlayerCommand method,
	// will loop through each line in the commands txt until it reaches the end,
	// and will print them to the screen.
